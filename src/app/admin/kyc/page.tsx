...
export default function AdminKYCPage() {
  ...
  const filteredSubmissions = submissions.filter((sub) => sub.kycStatus === activeTab);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 p-6">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-gray-900 mb-8">KYC Verification Dashboard</h1>
        ...
        <div className="space-y-4">
          {filteredSubmissions.length === 0 ? (
            <div className="bg-white rounded-lg p-12 text-center">
              ...
              <p className="text-gray-600">No {activeTab.toLowerCase()} submissions</p>
            </div>
          ) : (
                ...
                {expandedId === submission.id && (
                  ...
                  >
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2">Current Address</p>
                        <p className="text-gray-700 whitespace-pre-wrap">{submission.currentAddress}</p>
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2">Permanent Address</p>
                        <p className="text-gray-700 whitespace-pre-wrap">{submission.permanentAddress}</p>
                      </div>
                    </div>
                    ...
                    <div className="pt-4 border-t space-y-2 text-sm text-gray-600">
                      <p>Submitted: {new Date(submission.kycSubmittedAt).toLocaleString()}</p>
                      {submission.kycVerifiedAt && (
                        <p>Verified: {new Date(submission.kycVerifiedAt).toLocaleString()}</p>
                      )}
                      {submission.kycRejectionReason && (
                        <div className="bg-red-100 p-3 rounded-lg mt-3">
                          ...
                          <p className="text-red-800">{submission.kycRejectionReason}</p>
                        </div>
                      )}
                    </div>
                    ...
                    {submission.kycStatus === 'PENDING' && (
                      <div className="pt-6 border-t space-y-4">
                        ...
                        >
                          {processingId === submission.id ? 'Processing...' : 'Approve KYC'}
                        </button>
                        <div className="space-y-2">
                          ...
                          >
                            {processingId === submission.id ? 'Processing...' : 'Reject KYC'}
                          </button>
                        </div>
                      </div>
                    )}
