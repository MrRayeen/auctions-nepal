...
import { ArrowLeft, Share2, AlertCircle, Loader, ShieldCheck, Gavel, MessageCircle, Lock } from "lucide-react";
...
export default function AuctionDetailPage() {
    ...
    const fetchAuctionData = async () => {
      ...
      try {
        ...
        if (token) {
          ...
          const favStatusRes = await fetch(`/api/favorites/status?auctionId=${auctionData.id}`, {
        ...
        if (auctionData.tags) {
          try {
            // First, try to parse it as a valid JSON array
            const parsed = JSON.parse(auctionData.tags);
            setParsedTags(parsed);
          } catch (e1) {
            try {
              // If that fails, it might be concatenated JSON objects like {..}#{..}
              const sanitized = auctionData.tags.replace(/\\/g, '\\');
              const objects = sanitized.split('#').filter(Boolean).map(s => JSON.parse(s));
              setParsedTags(objects);
            } catch (e2) {
               try {
                // If that fails, it might be concatenated JSON objects like {..}{..}
                const objects = auctionData.tags.replace(/}\s*{/g, '},{');
  ...
  const handleQuickBid = (multiplier: number) => {
    ...
    const hasBidsAboveStart = (auction.bids || []).some(bid => bid.amount > auction.startingPrice);
  ...
  const handlePlaceBid = async () => {
    ...
    if (!userHasPin) {
      toastRef.current("You must set up a Transaction PIN in your profile before bidding.", "error");
      return;
    }
  ...
  const handlePinSubmit = async () => {
    ...
    try {
      ...
      toastRef.current(`Bid of Rs. ${amount.toLocaleString("en-IN")} placed successfully!`, "success");
    ...
    return (
      <main className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center p-8 rounded-2xl glass-panel">
          ...
          <p className="text-gray-400 mb-6">{error || "The auction you are looking for does not exist or has been moved."}</p>
          <Link href="/auctions" className="glass-button inline-flex items-center gap-2">
            <ArrowLeft size={16} />
  ...
  return (
    <main className="min-h-screen pb-24 relative overflow-x-hidden pt-8 md:pt-16">
      ...
      <Link href="/auctions" className="hidden fixed top-6 left-6 z-50 w-12 h-12 sm:flex items-center justify-center rounded-full glass-panel hover:bg-white/20 transition-all duration-300">
        <ArrowLeft size={24} />
      </Link>
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                ...
                <div className="flex gap-2 shrink-0">
                  <FavoriteButton auctionId={auction.id} isFavorited={isFavorited} onFavoriteChange={setIsFavorited} />
                  <ReportButton auctionId={auction.id} />
                  <button onClick={handleShare} className="w-12 h-12 flex items-center justify-center rounded-full glass-panel hover:bg-white/20 transition-colors" title="Share">
                    <Share2 size={22} />
                  </button>
                  {currentUserId !== auction.seller.id && (
                    <button onClick={handleChatWithSeller} className="w-12 h-12 flex items-center justify-center rounded-full glass-panel hover:bg-white/20 transition-colors" title="Chat with Seller">
            ...
            {auction.seller?.kycStatus === 'VERIFIED' && (
              <div className="flex items-center gap-2 text-sm text-nepal-accent font-semibold">
                <ShieldCheck size={18} />
                <span>Verified Seller: {auction.seller.name || auction.seller.email.split("@")[0]}</span>
              </div>
            )}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-4">
                {parsedTags.map((tag, idx) => {
                  ...
                  return (
                    ...
                    >
                      <p className="text-xs text-gray-400 uppercase font-bold tracking-wider">{key}</p>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 pt-6 border-t border-white/10">
              <div className="glass-panel p-4 rounded-xl">
                ...
                <p className="text-2xl font-bold text-white">Rs. {auction.startingPrice.toLocaleString("en-IN")}</p>
              </div>
              <div className="glass-panel p-4 rounded-xl">
                <p className="text-xs text-gray-400 uppercase">Total Bids</p>
                <p className="text-2xl font-bold text-white">{auction.bids.length || 0}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 relative">
          <div className="sticky top-24 space-y-6">
            {auction.status === 'ACTIVE' ? (
              <>
                <motion.div key={auction.currentPrice} initial={{ scale: 1 }} animate={{ scale: [1, 1.03, 1] }} transition={{ duration: 0.4 }} className="glass-panel p-8 rounded-3xl text-center relative overflow-hidden">
                ...
                <AutoBidSection auctionId={auction.id} currentPrice={auction.currentPrice} />

                <div className="glass-panel p-6 rounded-3xl space-y-4">
                  <label className="text-sm text-gray-400 uppercase tracking-wider">Place Your Bid</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[1, 2, 4].map(multiplier => (
                      <button key={multiplier} onClick={() => handleQuickBid(multiplier)} className="glass-button text-sm py-2">
                        +{(auction.minIncrement * multiplier).toLocaleString("en-IN")}
                      </button>
                    ))}
                  </div>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={`Min. Bid: Rs. ${(
                        auction.bids?.some(bid => bid.amount > auction.startingPrice)
                  ...
                  <button onClick={handlePlaceBid} disabled={isPlacingBid || !bidAmount} className="w-full glass-button py-4 text-lg font-bold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95 transition-transform">
                    {isPlacingBid ? <Loader size={20} className="animate-spin" /> : <Gavel size={20} />}
               ...
               <div className="glass-panel p-8 rounded-3xl text-center">
                    ...
                    <p className="text-gray-400">
                        {auction.status === 'SOLD' 
                            ? `This item was sold for Rs. ${auction.currentPrice.toLocaleString("en-IN")}`
            ...
            <div className="glass-panel rounded-3xl overflow-hidden min-h-[300px] flex flex-col">
              ...
              {displayBids.length > 0 ? <BidStream bids={displayBids} /> : <div className="flex-1 flex items-center justify-center text-gray-400 p-8 text-center">No bids yet. Be the first!</div>}
            </div>
          </div>
        </div>
      </div>
      ...
      {showPinModal && (
        ...
        >
          ...
          >
            ...
            <p className="text-gray-400 mb-6">Enter your 4-digit Transaction PIN to confirm this bid of Rs. {parseFloat(bidAmount || "0").toLocaleString("en-IN")}</p>
